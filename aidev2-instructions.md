...
- **Formatting:** All YAML files generated by the pipeline use 2-space indentation, block style, and no tabs. See the YAML Output Rules sections in the requirements and implementation instruction files.
- **Read efficiency:** Batch-read multiple YAML files in parallel when a step needs data from several sources. Cache file contents within a step and reuse across sequential sub-steps. Store per-type max sequence numbers in session memory after the first ID scan to avoid repeated full scans.
