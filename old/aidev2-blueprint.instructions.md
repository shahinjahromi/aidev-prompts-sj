...
- `01-requirements/01-pending-promotion/non_functional_requirements.yaml`
...
- `01-requirements/02-diff/non_functional/<ID>.yaml`
...
Requirements, contracts, and UI contracts may carry an optional `module` field (a lowercase hyphen-delimited slug, e.g. `auth`, `payments`, `accounts`).

### Reading requirements efficiently

When a task targets specific requirement IDs, read only the requirement items whose `module` matches the relevant modules:
- Before a full-file read, scan only the `id` + `module` fields to build a module index.
- Load full item details only for modules that contain requirements in scope.
- If an item has no `module` field, treat it as global scope (must always be loaded in full).

### Resolving the module of a replacement

When a requirement uses `action: update` and includes `replaces_id`:
1. Determine the replacement's `module` from the new entry's own `module` field (if present).
2. If the new entry has no `module` field, inherit the `module` from the `replaces_id` item in `03-current`.
3. If the module has changed (new entry has a different `module` than `replaces_id`), record the old module and new module — code in the old module location must be relocated or cleaned up as part of the implementation.

### Contract spec file location rule

Contract spec files (e.g. OpenAPI YAML, `ui_contracts.yaml`, database schemas) **must only be stored under `03-current/models_and_contracts/`**.
- Never write contract spec files into `01-pending-promotion/models_and_contracts/`.
- The `models_and_contracts.yaml` catalog file (which lists and references the specs) lives in `01-pending-promotion/` while pending, then in `03-current/` after promotion.
- All `spec_path` values in any `models_and_contracts.yaml` — whether that file lives in `01-pending-promotion/` or `03-current/` — must always use the path **`../03-current/models_and_contracts/<filename>`** so the reference resolves correctly from both locations.
